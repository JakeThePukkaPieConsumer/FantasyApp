...
router.post(
	...
	catchAsync(async (req, res) => {
		...
		if (
			providedKey.length !== secretKey.length ||
...
router.get(
	...
	catchAsync(async (req, res) => {
		...
		const [totalUsers, adminUsers, regularUsers, totalBudget] =
			await Promise.all([
				User.countDocuments(),
...
router.post(
	...
	catchAsync(async (req, res) => {
		...
		if (existingUser) {
			throw new AppError(
				`A user with this username already exists in ${targetYear}`,
				409
			);
		}
...
router.put(
	...
	catchAsync(async (req, res) => {
		...
		const actualUpdates = Object.keys(otherUpdates).filter((key) =>
		...
		if (otherUpdates.username && otherUpdates.username !== user.username) {
			const existingUser = await User.findOne({
				username: {
					$regex: new RegExp(`^${otherUpdates.username}$`, "i"),
				},
				_id: { $ne: userId },
			});
