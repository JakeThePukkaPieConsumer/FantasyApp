...
const { yearValidation, copyYearValidation, handleValidationErrors } = require('../middleware/validation');
...
// Initialize collections for a new year
router.post('/:year/initialize',
    ...
    catchAsync(async (req, res) => {
        ...
        if (existingStats && (
            ...
            throw new AppError(`Year ${year} already has existing data. Use copy endpoint to populate from another year.`, 409);
        }
...
// Copy data from one year to another
router.post('/copy',
    authenticateToken,
    checkRole('admin'),
    checkElevated,
    copyYearValidation,
    catchAsync(async (req, res) => {
        const { sourceYear, targetYear, collections = ['drivers', 'users'] } = req.body;
        
        if (sourceYear === targetYear) {
            throw new AppError('Source and target years cannot be the same', 400);
        }
        
        // Check if source year has data
        const sourceStats = await getYearStatistics(sourceYear).catch(() => null);
        if (!sourceStats) {
            throw new AppError(`Source year ${sourceYear} has no data to copy`, 404);
        }
        
        // Initialize target year if it doesn't exist
        try {
            await initializeYearCollections(targetYear);
        } catch (error) {
            // Collections might already exist, which is fine
            console.log(`Collections for ${targetYear} may already exist:`, error.message);
        }
...
// Compare two years
router.get('/:year1/compare/:year2',
    authenticateToken,
    [
        param('year1').matches(/^\d{4}$/).withMessage('Year1 must be a 4-digit number'),
        param('year2').matches(/^\d{4}$/).withMessage('Year2 must be a 4-digit number'),
    ...
    catchAsync(async (req, res) => {
        ...
        const [stats1, stats2] = await Promise.all([
            getYearStatistics(year1).catch(err => ({ year: parseInt(year1), error: err.message })),
            getYearStatistics(year2).catch(err => ({ year: parseInt(year2), error: err.message }))
        ]);
        ...
        const comparison = {
            drivers: {
                ...
                difference: {
                    count: (stats2.drivers?.count || 0) - (stats1.drivers?.count || 0),
                    totalValue: (stats2.drivers?.totalValue || 0) - (stats1.drivers?.totalValue || 0)
                }
            },
            users: {
                ...
                difference: {
                    count: (stats2.users?.count || 0) - (stats1.users?.count || 0),
                    totalBudget: (stats2.users?.totalBudget || 0) - (stats1.users?.totalBudget || 0)
                }
            },
            races: {
                ...
                difference: {
                    count: (stats2.races?.count || 0) - (stats1.races?.count || 0)
                }
            }
        };
...
// Delete all data for a specific year (dangerous!)
router.delete('/:year',
    authenticateToken,
    checkRole('admin'),
    checkElevated,
    ...
    [
        body('confirmDelete')
            ...
            .withMessage('Must provide confirmDelete: "DELETE_ALL_DATA" to proceed'),
        handleValidationErrors
    ],
    catchAsync(async (req, res) => {
        const year = req.params.year;
        ...
        // Extra protection for current year
        ...
        // Delete all collections for this year
        ...
        for (const collection of collections) {
            try {
                ...
                deletionResults[collection] = { success: true, dropped: result };
            } catch (error) {
