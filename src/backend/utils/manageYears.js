...
function showUsage() {
	...
	console.log(
		chalk.white("Usage: node manageYears.js <command> [options]\n")
	);

	console.log(chalk.cyan("Commands:"));
	console.log(
		chalk.white("  list                     - List all available years")
	);
	console.log(
		chalk.white(
			"  stats [year]             - Show statistics (all years or specific)"
		)
	);
	console.log(
		chalk.white(
			"  init <year>              - Initialize collections for a year"
		)
	);
	console.log(
		chalk.white("  copy <from> <to> [type]  - Copy data between years")
	);
	...
	console.log(
		chalk.white("  drivers,users            - Copy both (default)")
	);
	console.log(
		chalk.white(
			"  all                      - Copy drivers, users, and races\n"
		)
	);
			...
			try {
				...
				console.log(
					chalk.gray(
						`    Drivers: ${
							stats.drivers.count
						} (Total Value: £${stats.drivers.totalValue.toLocaleString()})`
					)
				);
				console.log(
					chalk.gray(
						`    Users: ${
							stats.users.count
						} (Total Budget: £${stats.users.totalBudget.toLocaleString()})`
					)
				);
...
async function showStats(year = null) {
	try {
		...
		console.error(
			chalk.red(`❌ Error getting statistics: ${error.message}`)
		);
	}
}
...
async function initializeYear(year) {
	try {
		...
		console.log(
			chalk.green(`✅ Successfully initialized collections for ${year}:`)
		);
		...
		console.error(
			chalk.red(`❌ Error initializing year ${year}: ${error.message}`)
		);
	}
}
...
async function copyData(fromYear, toYear, collections = "drivers,users") {
	try {
		...
		if (fromYear === toYear) {
			console.error(
				chalk.red("❌ Source and target years cannot be the same")
			);
			return;
		}
		...
		console.log(
			chalk.blue(`🔄 Copying data from ${fromYear} to ${toYear}...`)
		);
		console.log(
			chalk.gray(`   Collections: ${collectionsArray.join(", ")}`)
		);
		...
		try {
			...
			console.log(
				chalk.yellow(
					`⚠️  Target year ${toYear} collections may already exist`
				)
			);
		}
