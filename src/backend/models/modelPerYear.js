...
function getDriverModelForYear(year) {
    if (!validateYear(year)) {
        throw new Error(`Invalid year: ${year}. Must be between 2000 and ${new Date().getFullYear() + 5}`);
    }
...
function getRaceModelForYear(year) {
    if (!validateYear(year)) {
        throw new Error(`Invalid year: ${year}. Must be between 200 and ${new Date().getFullYear() + 5}`);
    }
    ...
    const model = mongoose.model(modelName, raceSchemaWithEvents, collectionName);
...
async function initializeYearCollections(year) {
    ...
    try {
        ...
        console.error(`Failed to initialize collections for year ${year}:`, err);
        throw err;
    }
}

async function copyYearData(sourceYear, targetYear, collections = ['drivers', 'users']) {
...
async function getYearStatistics(year) {
    ...
    try {
        ...
        const [
            ...
            Driver.aggregate([{ $group: { _id: null, total: { $sum: '$value' } } }]),
            User.aggregate([{ $group: { _id: null, total: { $sum: '$budget' } } }])
        ]);
