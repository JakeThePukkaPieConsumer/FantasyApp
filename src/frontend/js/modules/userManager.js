class UserManager {
	...
	async switchYear(year) {
		...
		if (!this.authModule.canAccessYear(year)) {
			this.notificationModule.error(
				"You do not have access to this year"
			);
			return;
		}
		...
		this.notificationModule.info(
			`Switched to ${this.authModule.formatYearDisplay(year)}`
		);
	}
	...
	async loadUsers() {
		try {
			const loadingNotification =
				this.notificationModule.showLoading("Loading users...");
			...
			this.notificationModule.error(
				`Failed to load users for ${this.authModule.formatYearDisplay(
					this.currentYear
				)}`
			);
		}
			...
			row.innerHTML = `
                <td colspan="5" class="text-center text-tertiary">
                    <div class="flex items-center justify-center gap-2 py-8">
                        <span>No users found for ${this.authModule.formatYearDisplay(
							this.currentYear
						)}</span>
                    </div>
                </td>
	...
	createUserRow(user) {
		...
		roleTd.innerHTML = `<span class="badge badge-${
			user.role === "admin" ? "primary" : "secondary"
		}">${user.role}</span>`;
		row.appendChild(roleTd);

		const budgetTd = document.createElement("td");
		budgetTd.textContent = `£${this.authModule.formatCurrency(
			user.budget
		)}`;
	...
	updateUserStats() {
		...
		if (totalBudgetEl || avgBudgetEl) {
			const totalBudget = this.users.reduce(
				(sum, user) => sum + (user.budget || 0),
				0
			);
			const avgBudget =
				this.users.length > 0 ? totalBudget / this.users.length : 0;

			if (totalBudgetEl) {
				totalBudgetEl.textContent = `£${this.authModule.formatCurrency(
					totalBudget
				)}`;
			}
			if (avgBudgetEl) {
				avgBudgetEl.textContent = `£${this.authModule.formatCurrency(
					avgBudget
				)}`;
			}
		}
		...
		if (yearDisplayEl) {
			yearDisplayEl.textContent = this.authModule.formatYearDisplay(
				this.currentYear
			);
		}
	}
	...
	async createUser(userData) {
		return await this.withElevation(async (token) => {
			...
			this.notificationModule.success(
				`User created successfully in ${this.authModule.formatYearDisplay(
					this.currentYear
				)}`
			);
			await this.loadUsers();
	...
	async updateUser(userId, userData) {
		return await this.withElevation(async (token) => {
			...
			this.notificationModule.success(
				`User updated successfully in ${this.authModule.formatYearDisplay(
					this.currentYear
				)}`
			);
			await this.loadUsers();
	...
	async deleteUserById(userId) {
		return await this.withElevation(async (token) => {
			...
			this.notificationModule.success(
				`User deleted successfully from ${this.authModule.formatYearDisplay(
					this.currentYear
				)}`
			);
			await this.loadUsers();
	...
	async resetUserPin(userId, newPin) {
		return await this.withElevation(async (token) => {
			const result = await this.apiModule.users.resetUserPin(
				userId,
				newPin,
				token,
				this.currentYear
	...
	deleteUser(userId, username) {
		this.modalModule.showConfirmation(
			`Are you sure you want to delete user "${username}" from ${this.authModule.formatYearDisplay(
				this.currentYear
			)}?`,
	...
	async searchUsers(query) {
		try {
			const result = await this.authModule.searchUsers(
				query,
				this.currentYear
			);
			if (result.success) {
	...
	getTotalBudget() {
		return this.users.reduce(
			(total, user) => total + (user.budget || 0),
			0
		);
	}
	...
	getAverageBudget() {
		return this.users.length > 0
			? this.getTotalBudget() / this.users.length
			: 0;
	}
